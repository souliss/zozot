...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	private static final String DATABASE_NAME = "ZozOtOEM.db";
	...
	public void onCreate(SQLiteDatabase db)
	{
		String sql = "CREATE TABLE {0} ({1} INTEGER PRIMARY KEY AUTOINCREMENT," + 
			" {2} TEXT NOT NULL,{3} TEXT NOT NULL, {4} TEXT NOT NULL,{5} INTEGER, {6} BOOLEAN);";
		db.execSQL(MessageFormat.format(sql, nodeTableInterface.TABLE_NAME, nodeTableInterface._ID,
				nodeTableInterface.NODE_NAME,nodeTableInterface.NODE_NUMBER,nodeTableInterface.DEVICE_NUMBER,nodeTableInterface.STREAM_NAME,nodeTableInterface.ENABLED ));
	...
	public void insertSoulissDevice(SQLiteDatabase db, String sNomeNodo, String sNumeroNodo, String sNumeroDevice, String sStreamName, boolean bEnabled)
	{
	...
	public ArrayList<String> readDBStreams(SQLiteDatabase readableDatabase, ArrayList<String> aXFeeds) {
		if (aXFeeds!= null && aXFeeds.size()>0 ) aXFeeds.clear();
		...
		try
		{
			...
			{
				aXFeeds.add(myCursor.getString(iColonna));
			}
		}
		finally
		{
	...
	public ArrayList<Device> readDBSoulissDevices(SQLiteDatabase readableDatabase, ArrayList<Device> aSoulissDevices) {
		...
		try
		{
			...
			while (myCursor.moveToNext())
			{
				Device d =new Device(Integer.parseInt(myCursor.getString(iColonna_NODE_NUMBER)), Integer.parseInt(myCursor.getString(iColonna_DEVICE_NUMBER)), myCursor.getString(iColonna_NODE_NAME));
